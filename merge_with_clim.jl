...
dfw = CSV.read( joinpath(pwd(),"data/processed/era5_700hpa_vertical_velocity.csv"), dateformat="yyyy-mm-ddTHH:MM:SS.s", DataFrame ) 
...
@select! df :Timestamp :lat :lon :Label :platform :w :lts :sst :aot1 
df = @orderby df :Timestamp
CSV.write( joinpath(pwd(),"data/processed/all_subtropic_label_w_lts_sst_aot.csv" ), df, index = false)
