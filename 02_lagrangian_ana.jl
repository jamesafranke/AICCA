...
df = @chain DataFrame( Arrow.Table( "./data/processed/transitions/all_transitions_SP.arrow" ) ) begin
    @subset :Label.==35
    @orderby :time_0 
    @by [:time_0, :lat, :lon] :class1=first(:Label) :class2=first(:next_label) :delta1=first(:hours) :id=first(:id) :id2=first(:next_id)
end
